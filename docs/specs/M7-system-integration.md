...
### Onboarding, selecting, leaving, the login item and hotkeys

- **The first launch** is Core's `onboardingPlan(record:ranBefore:isTranslocated:)` (`Onboarding.swift`, table-tested), from the preferences' `OnboardedVersion` (the build that ran onboarding, "0.1.0 (1)") and `LeftLivepaper`, from whether `library.json` or `app-state.json` is on disk, looked for as the app starts and before it writes anything, and from `/AppTranslocation/` in the path. Translocated: the move card alone, nothing registered or written. After leaving: the last card alone. Recorded by any version, or a library on disk with no record (a Livepaper from before onboarding, which is an update too): nothing. Otherwise the three cards. The record is written when the cards end, by their last button or the window's close button, keeping the first version that ran them; a Quit part way writes nothing, so the next launch shows them again, and the move card writes nothing ever.
- **The window** (`App/Screens/Onboarding`) holds one `OnboardingCard` whose values follow the step, in a window of its own (hidden title bar, the card's size, centred) that brings the Dock icon as the library window does. "Add a wallpaper": a drop anywhere on the window, under the library's `DropZoneOverlay`; the Open panel, as a sheet on it; or a sample, shown as a `WallpaperTile` that plays when the pointer rests on it, its poster the file's first frame. The import runs through the import list (`enqueueImport`), and the first wallpaper it gives, new or already there (`ImportList.batch`, tested), is set on All Displays before the card moves on. "Open at login" follows macOS's answer (`onboardingLoginPhase`, tested): on moves on; needs approval offers Login Items and moves on once the user allows it, the status being read again whenever the app becomes active or a window becomes key; not found and not registered say so; Not Now skips. "Make Livepaper your wallpaper" runs `Selection.select()` and is done on `.live`; on a fallback it gives `SelectionOutcome.words` with a button to open the pane again, and finishes when the click brings `.live`. From a fresh install that is four presses to a live wallpaper: a sample, Open at Login, Set as Wallpaper, Done.
...
- **Not selected.** The popover's line reads "Livepaper is not your wallpaper", shortened from "…not the wallpaper in System Settings" to make room, with "Open System Settings…" beside it as Restart sits beside its line (`statusLineOffersWallpaperPane`, tested: not while an import or Restart has the line). The store is not touched.
