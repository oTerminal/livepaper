...
| 9 | Fakes: `.recovering(.restartAgent)` shows the service not responding with Restart. Wired: not yet run |
| 10 | Not yet walked on screen; `FakeSystemServices` is tested |
| 11 | Partly: the accessibility tree was read over the popover, the window and Settings, which found the playlist picker's description ("Stack of rectangles") and the pause rules' missing labels; the keyboard found that Delete did nothing in the grid. All three are fixed in code and wait to be seen on screen, with the VoiceOver walk |
