...
- A new primary button lets go of the old one's focus first, in AppKit too (M7 review). Two glitches remained, found frame by frame at 0.1x. At card 2 to 3, Open at Login's ring stayed at its own width inside the wider Set as Wallpaper for the whole crossfade (about 1.9 s at 0.1x), dropped for a frame, then the new ring came in: clearing the focus state left the fading button's AppKit view first responder, and AppKit went on drawing its ring. And when the pane fallback finished card 3, Done opened with no ring and the focus on the window's hosting view: Open Wallpaper Settings had the focus, and the focused button a new title replaced took it back from the new one as it went. On every new primary button, of a new step or a new title, and when the card is disabled, the card now clears its focus state and makes the key window's first responder nil, then gives the new button the focus (`OnboardingCardFocus.delay`, tested: at once for a new title or a step changed by Return, after the crossfade for a step that fades). The focus value is the step and the title together, so a new title is a new value the focus moves to. Not yet checked on screen: the on-screen check was stopped while the user was using the Mac.
